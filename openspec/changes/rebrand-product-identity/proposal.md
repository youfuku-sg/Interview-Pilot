...
- 非対象: README.md / SECURITY.md(`rebrand-readme-security-ja` で対応済み)、UI内の日本語化そのもの(`localize-ui-japanese` で対応済み。本 change はブランド名の置き換えのみを扱う)、Pluely由来のライセンスキー課金・紹介マーケティング機能(Contribute/Promote/GetLicense/PluelyApiSetup)自体の削除要否(別スコープ、`localize-ui-japanese` 実施時にユーザーが「現状のまま日本語化する」と決定済み。名称表記の置き換えとは別軸の判断)。`Contribute.tsx`/`Promote.tsx` の pluely.com 本家プログラムへの案内文言・`support@pluely.com` 等の実在する連絡先も、実態と乖離させないため置き換え対象外
- 決定事項(2026-07-03): `productName`/パッケージ名は `Interview-Pilot`、`identifier` は `com.interview-pilot.app` に変更。`wix.language`/`nsis.languages` は日本語化。UI内の「Pluely」置き換え先はユーザー向け表示文言のみ「Interview-Pilot」または汎用名(「クラウドAPI」「推奨プロンプト」等)に統一し、関数名・コンポーネント名・ファイル名・localStorage キー名は対象外。詳細は design.md の Decisions を参照
