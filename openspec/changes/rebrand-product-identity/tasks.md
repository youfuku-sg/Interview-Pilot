...
- [ ] 1.5 MSIインストーラファイル名の `en-US` サフィックスについて、`bundle.windows.wix.language` を `ja-JP` に変更するかをユーザーに確認する(1.1〜1.3 とは独立に判断可能)
...
- [ ] 4.3 `wix.language` を `ja-JP` に変更した場合、CI上のビルドが問題なく完走し、MSIファイル名が `_ja-JP` になることを確認する。可能であればインストーラUIの表示言語も確認する
