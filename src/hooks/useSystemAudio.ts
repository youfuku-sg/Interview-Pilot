  ...
  const handleSetup = useCallback(async () => {
    try {
      ...
      if (hasAccess) {
        ...
        setSetupRequired(true);
        setError("権限が許可されていません。手動手順をお試しください。");
      }
    } catch (err) {
      setError("アクセス要求に失敗しました。以下の手動手順をお試しください。");
      setSetupRequired(true);
    }
  }, [startCapture]);
